...
class ApiRequest:
    ...
    def list_running_models(
        ...
        print('===================$$#')
        print(f'控制地址{data["controller_address"]}')
        response = self.post(
            "/llm_model/list_running_models",
            json=data,
        )
        print(f'hhhh{self._get_response_value(response, as_json=True, value_func=lambda r:r.get("data", []))}')
    ...
    def get_default_llm_model(self, local_first: bool = True) -> Tuple[str, bool]:
        ...
        def ret_sync():
            ...
            print(f'正在运行的模型:{running_models}')
