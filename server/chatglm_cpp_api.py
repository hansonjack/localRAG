...
class ChatCompletionRequestWithLanchainOpenAI(BaseModel):
    
    prompt:Literal[List[str],str]
    model: str = "default-model"
    temperature: float = Field(default=0.95, ge=0.0, le=2.0)
    max_tokens: int = Field(default=2048, ge=0)
    top_p: float = Field(default=0.7, ge=0.0, le=1.0)
    frequency_penalty: int = Field(default=0, ge=0)
    presence_penalty: int = Field(default=0, ge=0)
...
#修改-增
class ChatCompletionResponse4Langchain(BaseModel):
    id: str = "chatcmpl"
    model: str = "default-model"
    object: Literal["chat.completion", "chat.completion.chunk"]
    created: int = Field(default_factory=lambda: int(time.time()))
...
async def create_completion(body: ChatCompletionRequestWithLanchainOpenAI) -> ChatCompletionResponse:
    # ignore system messages
    ...
    #         'messages':history
    #     }
    #     generator = stream_chat_event_publisher(history, body1)
    #     return EventSourceResponse(generator)
